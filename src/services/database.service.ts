...
class DatabaseService {
  ...
  async getAllEventMessages(eventID: number) {
    return await knex<eventMessage>('eventmessages').select('eventmessages.*').innerJoin<userEvent>('user_event','eventmessages.User_Event_ID','user_event.ID').where('user_event.EventId',eventID);//user event id or event id
  }
}
